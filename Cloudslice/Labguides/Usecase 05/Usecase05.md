...
In today’s competitive digital marketplace, e-commerce companies generate large volumes of data from customer transactions, product catalogs, website interactions, and payment systems. Extracting meaningful insights from this data is essential for improving customer experience, optimizing operations, and increasing revenue. However, managing and analyzing large datasets from multiple sources can be complex without a unified analytics platform.

**Zava**, a rapidly growing e-commerce company, sells various consumer products through its online platform and processes thousands of orders every day. The company collects data from different systems including order management, customer profiles, product inventory, and payment transactions. As Zava’s business grows, the organization faces challenges in analyzing this data efficiently and providing real-time insights to business teams.

To address these challenges, Zava implements a modern analytics solution using **Microsoft Fabric**. Fabric provides a unified platform that integrates data engineering, data storage, data transformation, and business intelligence capabilities within a single environment. Zava stores its raw and processed e-commerce data in the Fabric Lakehouse, enabling scalable data management and analytics.

In addition, Zava leverages **Microsoft Fabric Data Agents** to enhance data accessibility and insights generation. Fabric Agents allow business users and analysts to interact with enterprise data using natural language queries. Instead of manually searching through reports or writing complex queries, users can simply ask questions such as:
...
The Fabric Agent automatically retrieves relevant data from the Lakehouse and generates insights, helping teams quickly understand business performance. This intelligent interaction improves productivity and enables faster decision-making across departments.

With this solution, Zava’s business users, analysts, and management teams can easily explore data, monitor key performance indicators, and gain real-time insights into sales performance, customer behavior, and product demand. By combining advanced analytics with AI-powered Fabric Agents, Zava builds a scalable and intelligent e-commerce analytics platform that supports data-driven growth and operational excellence.
...
1. Fabric home page, select **+New workspace** tile.
    ...
    ![A screenshot of a computer Description automatically generated](https://raw.githubusercontent.com/technofocus-pte/fbrciqdtagntsfrntr/refs/heads/main/Cloudslice/Labguide/Usecase%2005/media/image1.png)
...
In this task, you set up a lakehouse and ingest the NYC Taxi sample data along with additional CSV files. This establishes your raw dataset foundation inside Fabric, enabling you to start transformations and queries later.
...
1. On the **Filter by item type** search box, enter +++Lakehouse+++ and select the lakehouse item.

    ![A screenshot of a computer AI-generated content may be incorrect.](https://raw.githubusercontent.com/technofocus-pte/fbrciqdtagntsfrntr/refs/heads/main/Cloudslice/Labguide/Usecase%2005/media/image7.png)

1. On the **New lakehouse** dialog box, enter +++fabricagent_lakehouse+++ in the +++Name+++ field, click on the **Create** button and open the new lakehouse.
...
1. Wait for the notification stating **Successfully created SQL endpoint**.

    ![A screenshot of a computer AI-generated content may be incorrect.](https://raw.githubusercontent.com/technofocus-pte/fbrciqdtagntsfrntr/refs/heads/main/Cloudslice/Labguide/Usecase%2005/media/image10.png)

1. In the **lakehouse** page, navigate to **Get data in your lakehouse** section, and click on **Upload files**
...
1. Browse to **C:\LabFiles** on your VM, then select **customers.csv, Orders_Data.csv** and **products.csv** file and click on **Open** button.
...
1. Click and select refresh on the **Files**. The file appears.
...
1. In the **Lakehouse** page, Under the Explorer pane select Files. Now, however your mouse **Orders_Data.csv** file. Click on the horizontal ellipses **(…)** beside **Orders_Data.csv** . Navigate and click on **Load Table**, then select **New table**.
...
1. Repeat the same process for **customers.csv** and **products.csv** to convert them into tables.
...
1. Select **SQL analytics endpoint** from the **Lakehouse** drop-down menu at the top right of the screen.
...
1. From the lakehouse **Home** tab, select **New semantic model** and select the tables that you want to add to the semantic model.
...
1. In the **New semantic model** dialog enter +++E-commerce Order
...
1. From the left menu, select **Fabric-Copilot-@lab.LabInstance.Id** workspace icon and then select workspace name.
...
1. In the **Fabric-Copilot-@lab.LabInstance.Id** workspace page, navigate and click on **+New item** button, then select **Data agent**
...
1. Provide the name +++DataAgent_@lab.LabInstance.Id+++ and click **Create**
...
1. Select **E-commerce Order Dataset ** (Type: Semantic Model) from the results.
...
1. When you first ask the questions with the listed tables select **all tables** the data agent answers them fairly well.

1. For instance, for the question +++Who are the top 10 customers by total purchase amount?+++
...
1. Run the application and enter the sample questions to verify the responses.
...
1. Generate agent-level instructions using this meta-prompt in the **test pane** (on the right where it says **Test the agent's responses**):
...
1. Test the enhanced agent with more complex queries:
...
1. Click **Publish** and paste the generated description in the purpose and capabilities field.
...
1. Click the blue **+Create blank agent** button.
...
1. Configure your agent with these settings:
...
1. Click **Connect to an external agent** and select **Microsft Fabric (preview)**.
...
1. If it say's *Connection : Not connected* then click the drop down next to *Not connected* and select **Create new connection**. Verify it's showing as the email for your account, then click **Next**.
...
1. Click **Create** and sign in using the same account used for this lab
...
1. Select your Fabric Data Agent
...
1. Enter the **agent name** as +++DataAgent-@lab.LabInstance.Id+++, verify the **connection**, and then click **Add and configure** to proceed with the agent setup.
...
1. Click **Publish** to make the agent available for use
...
1. Click **Allow** to grant the required permissions
    ...
    >[!Note] The response generation process may take **5–6 minutes** to complete.
...
1. Select **Teams and Microsoft 365 Copilot**
...
1. Select **See agent in Teams** to open and test the agent in **Microsoft Teams**.
...
1. Enter your provided credentials to sign in and continue
...
1. After the app is added successfully, click the *Open* button to launch the E‑commerce RAG Agent in Microsoft Teams
...
This use case focuses on how an e‑commerce organization can integrate **Microsoft Fabric Data Agents** with **Microsoft Teams** through **Copilot Studio** to deliver real‑time insights, natural‑language analytics, and multi‑agent collaboration. By combining a unified analytics platform (Microsoft Fabric) with conversational AI (Copilot Studio and Teams), business users can seamlessly access sales trends, product insights, and customer behavior without writing queries. The solution demonstrates how AI agents can retrieve data from Fabric Lakehouse, enrich responses using instructions, and work alongside other agents to streamline business intelligence workflows.
